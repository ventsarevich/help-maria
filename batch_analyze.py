...
<span>анализаторы: {html.escape(item.analyzers or 'yoloe')}</span><span>рейтинг {item.score:.3f}</span><button onclick="openViewer({index})">Открыть и приблизить</button></div></article>''')
    ...
    viewer_data = json.dumps([{"frame": item.frame_image, "crop": item.crop_image, "video": item.video,
...
<div id="modal"><div class="viewerbar"><button onclick="move(-1)">← Предыдущий</button><button onclick="move(1)">Следующий →</button><button onclick="setSource('frame')">Кадр</button><button onclick="setSource('crop')">Фрагмент</button><b id="caption"></b><button id="close">Вернуться ×</button></div><img id="large"><div class="hint">← → навигация · колесо или +/− зум до 40× · перетаскивание · Esc назад</div></div><script>
...
def main():
    ...
    frames_dir, crops_dir = run_dir / "frames", run_dir / "crops"
